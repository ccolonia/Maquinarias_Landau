...
function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        // Redirigir a la página de bienvenida con el nombre del lead
        const encodedName = encodeURIComponent(formData.name)
        const leadId = data.id || ''
        window.location.href = `/bienvenida?nombre=${encodedName}&id=${leadId}`
      } else {
...
function FlipCard({ service, index }: { service: Service; index: number }) {
  ...
  // Safety checks para propiedades del servicio
  const safeService = {
  ...
  const Icon = iconMap[safeService.icon] || Wrench
  
  // Safe parsing de features con manejo de errores
  let features: string[] = []
  try {
    const parsed = typeof safeService.features === 'string' 
      ? JSON.parse(safeService.features) 
      ...
      <div className={`flip-card-inner ${isFlipped ? 'transform rotate-y-180' : ''}`}>
        ...
        <div className="flip-card-front">
          <div className="relative w-full h-full">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
              ...
              <h3 className="text-xl font-bold text-white drop-shadow-lg">{safeService.title}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="flip-card-back bg-white p-6 flex flex-col justify-center border border-gray-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-xl font-bold text-gray-900">{safeService.title}</h3>
          </div>
          <p className="text-gray-600 text-sm leading-relaxed mb-6">{safeService.description}</p>
...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch de datos públicos (sin cache option en client-side)
        ...
        // Parsear respuestas de forma segura
  ...
  // Métricas desde configuración con valores por defecto
  const metrics = [
    { value: config?.yearsExperience || 75, suffix: "+", label: "Años de Experiencia" },
    { value: config?.clientsCount || 50000, suffix: "+", label: "Clientes Satisfechos" },
    { value: config?.brandsCount || 200, suffix: "+", label: "Marcas Disponibles" },
    { value: config?.techniciansCount || 15, suffix: "", label: "Técnicos Certificados" }
  ]
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {Array.isArray(testimonials) && testimonials.length > 0 ? testimonials.map((testimonial, index) => {
              ...
              return (
                <Card key={testimonial?.id || index} className="bg-white border border-gray-100 animate-on-scroll" style={{ animationDelay: `${index * 100}ms` }}>
                  <CardContent className="p-6">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-semibold text-gray-900">{testimonial?.name || 'Usuario'}</div>
              ...
              <div className="space-y-5">
                ...
                {config?.whatsapp && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a 
                        href={`https://wa.me/${config.whatsapp}?text=${encodeURIComponent('Gracias por comunicarte con Maquinarias Landau ¿Cómo podemos ayudarte?')}`}
                        target="_blank"
                        rel="noopener noreferrer"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {(() => {
                      try {
                        const schedule = config?.schedule ? JSON.parse(config.schedule) : null
                        if (schedule) {
                          return (
                            <>
                              {schedule.weekdays && <p className="text-gray-600 text-sm">{schedule.weekdays}</p>}
                              {schedule.friday && schedule.saturday && (
                                <p className="text-gray-600 text-sm">{schedule.friday} | {schedule.saturday}</p>
                              )}
                              {schedule.friday && !schedule.saturday && (
                                <p className="text-gray-600 text-sm">{schedule.friday}</p>
                              )}
                              {!schedule.friday && schedule.saturday && (
                                <p className="text-gray-600 text-sm">{schedule.saturday}</p>
                              )}
                            </>
                          )
                        }
                      } catch (e) {
                        // Si falla el parse, mostrar el texto plano
                        if (config?.schedule) {
                          return <p className="text-gray-600 text-sm whitespace-pre-line">{config.schedule}</p>
                        }
                      }
                      // Default si no hay schedule
                      return (
                        <>
      ...
      {/* WhatsApp Floating Button - MOVED LEFT */}
      <a
        href={`https://wa.me/${config?.whatsapp || '5491162422197'}?text=${encodeURIComponent('Gracias por comunicarte con Maquinarias Landau ¿Cómo podemos ayudarte?')}`}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 left-6 z-50 w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-300 group"
