...
export default function ProductosPage() {
  ...
  const [whatsappNumber, setWhatsappNumber] = useState<string>('5491162422197')
    ...
    async function fetchData() {
      try {
        ...
        if (configData?.whatsapp) {
          setWhatsappNumber(configData.whatsapp)
        }
  ...
  // Parsear features de forma segura (soporta JSON y texto plano)
  const parseFeatures = (featuresData: string): string[] => {
    try {
      if (featuresData) {
        const parsed = typeof featuresData === 'string' 
          ? JSON.parse(featuresData) 
          : featuresData
        if (Array.isArray(parsed)) return parsed
      }
    } catch {
      // Si no es JSON válido, intentar parsear como texto plano (una característica por línea)
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center gap-8">
              ...
              <a href="/productos" className="text-gray-800 font-semibold border-b-2 border-[#BE1E2D]">Productos</a>
              <a href="/catalogo" className="text-gray-500 hover:text-gray-800 transition-colors">Catálogo</a>
        ...
        {isMenuOpen && (
          <div className="lg:hidden bg-white/95 backdrop-blur-xl border-t border-gray-200">
            <div className="px-4 py-6 space-y-4">
              ...
              <a href="/productos" className="block text-gray-900 font-semibold py-2">Productos</a>
              <a href="/catalogo" className="block text-gray-600 hover:text-gray-900 py-2">Catálogo</a>
          ...
          >
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className="grid md:grid-cols-2 gap-0">
                {/* Imagen */}
                <div className="relative aspect-square md:aspect-auto md:h-[400px] bg-gradient-to-br from-gray-50 to-gray-100 flex-shrink-0">
                  ...
                  <div className="absolute top-4 left-4 flex flex-col gap-2">
                    {selectedProduct.category?.name && (
                      <Badge 
                        ...
                        style={{ backgroundColor: selectedProduct.brand?.color || '#BE1E2D', color: 'white' }}
                      >
                    ...
                    {selectedProduct.power && (
                      <Badge variant="outline" className="text-sm px-3 py-1 bg-white/95">
                ...
                {/* Info con scroll */}
                <div className="p-6 md:p-8 flex flex-col overflow-y-auto max-h-[400px] md:max-h-[400px]">
                  {/* Marca */}
                  {selectedProduct.brand?.name && (
                    <div className="flex items-center gap-2 mb-3 flex-shrink-0">
                      <div 
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-sm"
                        style={{ backgroundColor: selectedProduct.brand?.color || '#BE1E2D' }}
                      >
                        {selectedProduct.brand.name.charAt(0)}
                      </div>
                      <span className="text-sm font-medium text-gray-600">{selectedProduct.brand.name}</span>
                    </div>
                  )}

                  {/* Nombre */}
                  <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 flex-shrink-0">
                  ...
                  {/* Características */}
                  {parseFeatures(selectedProduct.features).length > 0 && (
                    <div className="mb-6 flex-shrink-0">
                      <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                        <Tag className="w-4 h-4 text-[#BE1E2D]" />
                        Características
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {parseFeatures(selectedProduct.features).map((feature: string, idx: number) => (
                          <Badge 
                            key={idx} 
                            variant="outline" 
                            className="text-sm px-3 py-1.5 bg-gray-50 border-gray-200 text-gray-700"
                          >
                            {feature}
                          </Badge>
                        ))}
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6 flex-shrink-0">
                    {selectedProduct.power && (
                      <div className="bg-gray-50 rounded-xl p-4">
                        ...
                        <p className="font-semibold text-gray-900">{selectedProduct.power}</p>
                      </div>
                    )}
                    {selectedProduct.brand?.name && (
                      <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  {/* Botón de WhatsApp */}
                  <div className="mt-auto flex-shrink-0 pt-4 border-t border-gray-100">
                    <a
                      href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hola! Me interesa el producto: ${selectedProduct.name}\n\nMarca: ${selectedProduct.brand?.name || 'N/A'}\nCategoría: ${selectedProduct.category?.name || 'N/A'}\nPotencia: ${selectedProduct.power || 'N/A'}\n\n¿Podrían darme más información?`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full block"
                    >
                      <Button className="w-full bg-[#25D366] hover:bg-[#20BD5A] text-white py-3 text-base">
      ...
      <a
        href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent('Gracias por comunicarte con Maquinarias Landau ¿Cómo podemos ayudarte?')}`}
        target="_blank"
        rel="noopener noreferrer"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-6">
              ...
              <a href="/catalogo" className="text-gray-500 hover:text-gray-800 text-sm transition-colors">Ver Catálogos</a>
